...
def main():
    ...
    if config.run.distributed and dist.is_initialized() and dist.get_world_size() > 1:
        collate_fn_1 = getattr(imagenet_data, "collater", None)
        imagenet_datasampler = DistributedSampler(
            imagenet_data,
        ...
        collate_fn_2 = getattr(imagenet_data, "collater", None)
        target_data_sampler = DistributedSampler(
            target_data,
            num_replicas=dist.get_world_size(),
            rank=dist.get_rank(),
            shuffle=False
        ...
        imagenet_datasampler = None
        target_data_sampler = None    

    data_loader_imagenet = torch.utils.data.DataLoader(imagenet_data, 
                                                       batch_size=args.batch_size, 
                                                       shuffle=False, num_workers=config.run.num_workers, 
                                                       drop_last=False, pin_memory=True, 
                                                       sampler=imagenet_datasampler,
                                                       collate_fn=collate_fn_1)
    
    data_loader_target   = torch.utils.data.DataLoader(target_data, batch_size=args.batch_size, 
                                                                           shuffle=False, num_workers=config.run.num_workers, 
                                                                           drop_last=False, pin_memory=True, 
                                                                           sampler=target_data_sampler,
